#!/usr/bin/env node
/**
 * Smoke test do pacote em ESM: empacota o tarball, instala num projeto limpo e
 * importa 'uesp-eso-build-wrapper' via `import` (ESM) e `require` (CJS),
 * inicializando o motor com o game data do repo e calculando um build.
 *
 * É o teste real de resolução do dual build:
 *   - exports["."].import  → dist/esm (ESM de verdade, via import.meta.dirname)
 *   - exports["."].require → dist/lib (CJS, via __dirname)
...
for (const rel of ['dist/lib/eso-engine/index.js', 'dist/esm/lib/eso-engine/index.js']) {
  if (!fs.existsSync(path.join(repoRoot, rel))) {
    console.error(`[test-esm] build faltando: ${rel} — rode npm run build primeiro`);
    process.exit(1);
  }
}

...
exports.cacheStatObjects = cacheStatObjects;
exports.calculateBuild = calculateBuild;
exports.listAvailableBuffs = listAvailableBuffs;
exports.listRacialPassives = listRacialPassives;
exports.listClassPassives = listClassPassives;
exports.listPassivesBySkillLine = listPassivesBySkillLine;
exports.listAvailableSkillLines = listAvailableSkillLines;
exports.listAvailableToggleSkills = listAvailableToggleSkills;
...
let _statObjects = null;
function cacheStatObjects() {
    const stats = global.g_EsoComputedStats;
...
function calculateBuild(input) {
    const { character, items, championPointNodes, activeBuffs, toggleSkills, skillBars, activeWeaponBar, passiveSkills, autoPassives, enchantOverrides, toggledSetBonuses, } = input;
    ...
    const buffData = global.g_EsoBuildBuffData;
    const toggleSkillData = global.g_EsoBuildToggledSkillData;
    ...
    if (buffData && typeof buffData === 'object') {
        for (const key of Object.keys(buffData)) {
            ...
            if (b && typeof b === 'object') {
                ...
                if (b.maxTimes != null && b.count == null)
                    b.count = 0;
            }
        }
    }
    ...
    global.g_EsoSkillBarData = [emptyBar(), emptyBar()];
    ...
    global._esoWrapperTwiceBornOverride = !!character.mundusStone2;
    if (character.mundusStone2) {
        (0, env_setup_1.setDomValue)('esotbMundus2', character.mundusStone2);
    }
    ...
    (0, env_setup_1.setDomValue)('esotbCPTotalPoints', String(character.championPoints ?? 0));
    ...
    (0, env_setup_1.setDomValue)('esotbTargetResistance', '18200'); // UESP default: CP160 enemy base resistance
    (0, env_setup_1.setDomValue)('esotbTargetEffectiveLevel', '66'); // UESP default: 66 = CP160 (endgame content)
    (0, env_setup_1.setDomValue)('esotbTargetCritResistFlat', '0');
    (0, env_setup_1.setDomValue)('esotbTargetPenetrationFlat', '0');
    (0, env_setup_1.setDomValue)('esotbTargetPenetrationFactor', '0');
    (0, env_setup_1.setDomValue)('esotbTargetDefenseBonus', '0');
    (0, env_setup_1.setDomValue)('esotbTargetAttackBonus', '0');
    (0, env_setup_1.setDomValue)('esotbTargetCritDamage', '0');
    (0, env_setup_1.setDomValue)('esotbTargetCritChance', '0');
    (0, env_setup_1.setDomValue)('esotbTargetPercentHealth', '100');
    // -------------------------------------------------------------------------
    ...
    if (enchantData && enchantOverrides) {
        for (const slot of ALL_SLOTS) {
            ...
            if (override)
                enchantData[slot] = { ...override };
        }
    }
    const cpDataGlobal = global.g_EsoCpData;
    ...
    if (championPointNodes && Object.keys(championPointNodes).length > 0) {
        (0, env_setup_1.setDomValue)('esotbEnableCP', 'true');
        const hasCpRules = !!global.g_EsoBuildRules?.cp;
        const cpSkills = global.g_EsoCpSkills ?? {};
        const cpSkillDesc = global.g_EsoCpSkillDesc ?? {};
        ...
        for (const [nodeId, nodeData] of resolvedNodes) {
            if (hasCpRules) {
                ...
                let desc = nodeData.description;
                if (!desc && cpSkillDesc[nodeId]) {
                    const nodeDescMap = cpSkillDesc[nodeId];
                    ...
                    if (points !== undefined) {
                        ...
                        if (!desc) {
                            ...
                            if (floorKey !== undefined)
                                desc = nodeDescMap[String(floorKey)];
                        }
                    }
                    if (!desc) {
                        desc = nodeDescMap[0] ?? nodeDescMap['0'] ?? Object.values(nodeDescMap)[0];
                    }
                }
                if (desc) {
                ...
                const bonusStr = typeof bonus === 'string' && bonus.endsWith('%')
                    ? `Current value: ${bonus}`
                    : `Current bonus: ${bonus}`;
                (0, env_setup_1.setDomAttr)(`skill_${nodeId}`, 'unlocked', '1');
                (0, env_setup_1.setDomTextContent)(`descskill_${nodeId}`, bonusStr);
                (0, env_setup_1.setDomTextContent)(`descskill_${nodeId}_prev`, `CP Node ${nodeId}`);
            }
        }
    }
    // -------------------------------------------------------------------------
    ...
    if (toggleSkills) {
        for (const skillName of toggleSkills) {
            if (toggleSkillData) {
                if (!toggleSkillData[skillName])
                    toggleSkillData[skillName] = {};
    ...
    if (skillBars) {
        ...
        for (const [slots, barIndex] of barMap) {
            ...
            slots.slice(0, 6).forEach((slot, slotIndex) => {
                global.g_EsoSkillBarData[barIndex][slotIndex] = {
    ...
    // -------------------------------------------------------------------------
    const allPassiveIds = new Set(passiveSkills ?? []);
    if (autoPassives) {
        // Use the loader snapshot (written before any calculations) to avoid raceType mutation.
        const snapshot = global.g_EsoPassiveSkillSnapshot;
        if (snapshot) {
    ...
    if (allPassiveIds.size > 0) {
        ...
        global.g_EsoSkillPassiveData = passiveData;
    }
    // -------------------------------------------------------------------------
    ...
    if (skillBars) {
        ...
        global.g_EsoSkillActiveData = activeData;
    }
    // -------------------------------------------------------------------------
    ...
    global.g_EsoBuildActiveWeapon = activeBar;
    global.g_EsoBuildActiveAbilityBar = activeBar;
    // -------------------------------------------------------------------------
    ...
    const origIsEnabled = global.IsEsoBuildToggledSetEnabled;
    if (toggledSetIds.size > 0) {
        global.IsEsoBuildToggledSetEnabled = function (setId) {
            if (toggledSetIds.has(String(setId))) {
                const td = global.g_EsoBuildToggledSetData?.[setId];
                if (td?.valid)
                    return true;
            }
    ...
    try {
        ...
        if (toggledSetIds.size > 0) {
            global.IsEsoBuildToggledSetEnabled = origIsEnabled;
        }
    }
    // -------------------------------------------------------------------------
 ...
 */
function listAvailableBuffs(group) {
    const buffData = global.g_EsoBuildBuffData;
    if (!buffData || typeof buffData !== 'object')
        return [];
    const result = [];
    for (const [name, entry] of Object.entries(buffData)) {
        if (!entry || typeof entry !== 'object')
            continue;
        const entryGroup = entry.group ?? entry.groupName ?? '';
        if (group !== undefined && entryGroup !== group)
            continue;
...
function buildPassiveSkillInfos(filter, sortKey) {
    const snapshot = global.g_EsoPassiveSkillSnapshot;
 ...
 */
function listRacialPassives(race) {
    return buildPassiveSkillInfos((v) => v.raceType === race, (v) => v.baseName);
}
/**
 ...
 */
function listClassPassives(className) {
    return buildPassiveSkillInfos((v) => v.classType === className, (v) => (v.skillLine ?? '') + v.baseName);
}
/**
 ...
 *   "Vampire", "Werewolf"
 */
function listPassivesBySkillLine(skillLine) {
    return buildPassiveSkillInfos((v) => v.skillLine === skillLine, (v) => v.baseName);
}
/**
 ...
 * Must be called after initEsoEngineFromData().
 */
function listAvailableSkillLines() {
    const snapshot = global.g_EsoPassiveSkillSnapshot;
    if (!snapshot || typeof snapshot !== 'object')
        return [];
    const lines = new Set();
...
function listAvailableToggleSkills() {
    ...
    if (!toggleData || typeof toggleData !== 'object')
        return [];
    const result = [];
    for (const [name, entry] of Object.entries(toggleData)) {
        if (!entry || typeof entry !== 'object')
            continue;
        if (!name)
            continue; // skip anonymous entries (no nameId in the rule)

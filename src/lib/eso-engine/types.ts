...
export interface BuildInput {
  ...
  /**
   * When true, automatically injects the game's inherent passives — the
   * `ESO_FREE_PASSIVES` list (Light/Medium/Heavy Armor Bonuses and Penalties,
   * racial, craft, and a few free actives) — in addition to any explicit
   * `passiveSkills` and `autoPassives`.
   *
   * Mirrors the UESP Build Editor, which loads this baseline by itself. The
   * armor entries are gated per equipped piece (`factorStatId`
   * ArmorLight/ArmorMedium/ArmorHeavy), so with no armor of that weight they
   * contribute nothing. Use `listInherentPassives()` to enumerate them.
   *
   * @default false
   */
  autoInherentPassives?: boolean;
  /**
